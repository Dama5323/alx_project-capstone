...
const ForecastChart = ({ forecastData, unit }) => {
  ...
  return (
    <div className="mt-10 animate-in fade-in slide-in-from-bottom-5 duration-1000">
      ...
      {/* Mobile: Horizontal Scroll with visible indicator */}
      <div className="relative">
        {/* Scroll hint fade effects */}
        <div className="absolute left-0 top-0 bottom-0 w-8 bg-gradient-to-r from-white dark:from-gray-900 to-transparent pointer-events-none z-10"></div>
        ...
        <div className="flex overflow-x-auto gap-4 pb-6 px-4 -mx-4 scrollbar-hide snap-x snap-mandatory">
          {dailyForecasts.map((day, index) => (
            <div 
              key={index} 
              className="min-w-[160px] snap-start bg-gray-100/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl p-5 border border-gray-300 dark:border-gray-700 flex flex-col items-center shadow-md hover:shadow-lg transition-all duration-300"
            >
        ...
        <div className="flex justify-center gap-1.5 mt-2 sm:hidden">
          {dailyForecasts.map((_, i) => (
            <div 
              key={i} 
              className={`w-2 h-2 rounded-full transition-all duration-300 ${
                i === 0 ? 'bg-blue-500 w-4' : 'bg-gray-300 dark:bg-gray-600'
              }`}
            />
          ))}
        </div>
